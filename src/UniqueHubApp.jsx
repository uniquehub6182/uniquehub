...
function CalendarPage({ onBack }) {
  ...
  const deleteEvent = (id) => { setEvents(p=>p.filter(e=>e.id!==id)); setViewEvent(null); showToast("Evento removido"); };
...
function IdeasPage({ onBack }) {
  ...
  const vote = (id) => setIdeas(p => p.map(i => i.id===id ? {...i, votes:i.votes+1} : i));
