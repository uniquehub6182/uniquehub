...
serve(async (req) => {
  ...
  try {
    ...
    if (isReels) {
      /* ── REELS: Facebook Reels API using file_url (no download needed) ── */
      const videoUrl = urls[0];
      if (!videoUrl) throw new Error("No video URL for Reels");
      console.log(`[FB Reels] Publishing with file_url: ${videoUrl.substring(0, 100)}`);
      ...
      const res = await fetch(`https://graph.facebook.com/v21.0/${page_id}/video_reels`, {
        method: "POST", body: params
      });
      const data = await res.json();
      console.log("[FB Reels] Response:", JSON.stringify(data).substring(0, 300));
      if (data.error) throw new Error(data.error.message || "Reels publish failed");

      return json({
        success: true,
        media_id: data.id || data.video_id,
