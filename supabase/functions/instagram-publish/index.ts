...
serve(async (req) => {
  ...
  try {
    ...
    if (!client_id || !urls.length) throw new Error("Missing client_id or images");
    ...
    const label = carousel ? "Carrossel" : type === "STORIES" ? "Story" : "Post";
    return json({ success: true, media_id: pd.id, media_type: carousel ? "CAROUSEL" : type, message: `${label} publicado!` });
